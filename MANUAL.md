...
### Semantic Entry Points (`--semantic-query`)

All function-targeted commands support natural-language entry point discovery via HNSW embeddings generated by Magellan.

**Prerequisite:** Run `magellan embed --db <db>` to create the HNSW index before using semantic queries.

```bash
# Instead of an exact function name, describe what you want to analyze
mirage paths --semantic-query "error handling in the parser"
mirage cfg --semantic-query "config validation"
mirage dominators --semantic-query "network retry logic"
```

| Option | Description |
|--------|-------------|
| `--semantic-query <TEXT>` | Natural-language query resolved via HNSW embeddings |
| `--file <PATH>` | File path filter applied to semantic results (optional) |

**How it works:**
1. Embeds the query via Ollama (endpoint configured in `~/.config/magellan/config.toml`)
2. Searches the HNSW index in the database for closest symbol vectors
3. Returns the top matching function as the entry point for analysis

**Error handling:** If no HNSW index exists, Mirage reports:
```
