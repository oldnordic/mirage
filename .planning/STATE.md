...
**From 12-01 (Magellan CFG Integration):**
- Magellan's cfg_blocks table (v7+) replaces Charon for CFG extraction
- Schema migration: `block_kind` → `kind`, `terminator` JSON → TEXT, added `start_line/start_col/end_line/end_col`
- Terminator parsing: string matching instead of JSON deserialization (fallthrough/conditional/goto/return/break/continue/call/panic)
- Source locations: use Magellan's line/column data directly, no byte offset conversion
- Path caching simplified: removed `function_hash` column, Magellan manages its own cache invalidation
- Error messages updated: "mirage index" → "magellan watch"
- MIN_MAGELLAN_SCHEMA_VERSION updated from 4 to 7
- External Charon dependency completely removed
- All 408 tests pass with new schema

**From 12-02 (Edge Construction from Terminators):**
- Edges are now derived in memory from terminator strings using build_edges_from_terminators() (RESEARCH.md Pattern 2)
- No longer querying cfg_edges table for edge data (kept for backward compatibility only)
- Terminator to edge mapping: conditional→TrueBranch/FalseBranch, fallthrough→Fallthrough, goto→Fallthrough, return→no edges
- store_cfg() deprecated with clear documentation that Magellan handles CFG storage
- DatabaseStatus::cfg_edges field marked as deprecated
- Simplified sequential edge resolution (will be enhanced with proper target resolution in future)
- All 408 tests pass with new edge construction approach

**From 12-03 (Charon Removal):**
- Entire Charon/ULLBC integration removed from codebase (src/mir/charon.rs, src/cfg/mir.rs)
- `mirage index` command removed - Magellan watch is the only indexing tool
- src/mir/mod.rs kept as reserved module for future AST-based MIR alternatives
- CharonSpan and from_charon_span removed from cfg/source.rs
- No external binary dependencies remain for CFG extraction
- All 394 tests pass after Charon removal

**From 12-04 (Documentation Cleanup):**
- All Charon references removed from user-facing documentation (README.md, ARCHITECTURE.md, CONCEPT.md, ECOSYSTEM_CONTEXT.md)
- Quick Start updated to emphasize `magellan watch` as workflow starting point
- Mirage repositioned as read-only analyzer powered by Magellan AST-based CFG
- Historical Charon references kept in ROADMAP/LIMITATIONS_AND_ROADMAP.md to document architecture migration
- All CLI commands verified working with Magellan CFG data (paths, cfg, dominators, loops, patterns, frontiers, blast-zone, cycles, slice, hotspots)

**From 12-05 (Test Updates for Magellan v7):**
- Test helpers create Magellan v7-compatible databases with explicit cfg_blocks table
- All test fixtures use lowercase kind/terminator strings ("entry", "block", "return", "fallthrough", "conditional")
- function_hash column removed from tests (Magellan v7 doesn't use this column)
- Integration tests updated to use `mirage_analyzer` import (not `mirage`)
- create_schema() requires magellan_schema_version parameter (all tests pass 7 for Magellan v7)
- Added Debug derive to MirageDb for better error messages in tests
- Graceful degradation tests verify helpful error messages when CFG data missing

**From 13-01 (Dual Backend Support):**
- Compile-time exclusivity via compile_error! macro when both sqlite and native-v2 features enabled
- File header detection: SQLite databases identified by "SQLite format 3" header; native-v2 format is everything else
- Internal feature naming pattern: backend-sqlite/backend-native-v2 with public aliases sqlite/native-v2 for backward compatibility
- Backend validation at MirageDb::open() provides helpful rebuild instructions when format mismatches compile-time feature
- rusqlite still used for SQLite backend; GraphBackend integration for native-v2 deferred to future work

**From 14-01 (GraphBackend Integration):**
- MirageDb refactored to wrap Box<dyn GraphBackend> instead of direct rusqlite::Connection
- Runtime backend detection uses Backend::detect() to select GraphConfig::sqlite() or GraphConfig::native()
- sqlitegraph::open_graph() factory creates backend-agnostic database connection
- Adapter methods conn()/conn_mut() return Result<&Connection> for backward compatibility
...
**Phase 11 added (2026-02-03):** Inter-procedural Dominance, Hotspots & Smart Re-indexing - COMPLETE
...
- Completed: All 6 plans (11-01 through 11-06)

**Phase 12 added (2026-02-04):** Magellan CFG Integration - COMPLETE
- Trigger: Charon external dependency creates user installation burden
- Purpose: Replace Charon-based CFG extraction with Magellan's AST-based cfg_blocks table (v7+)
- Completed: All 5 plans (12-01 through 12-05)
- Results:
  * Magellan v7 schema integration (cfg_blocks with kind/terminator as lowercase TEXT)
  * Edge construction from terminator data (no stored edges)
  * Charon removal (dependencies, imports, docs, code)
  * Documentation cleanup (README, ARCHITECTURE, CONCEPT)
  * Test updates to use Magellan v7 schema

**Phase 13 added (2026-02-10):** Dual Backend Support - sqlite and native-v2 - COMPLETE
- Trigger: sqlitegraph 1.5.7 and magellan 2.2.1 released with native-v2 I/O layer support
- Purpose: Add dual backend support allowing users to choose between sqlite-backend (default) and native-v2 (newer I/O layer)
- Completed: All 1 plan finished (13-01)
- Results:
  * Compile-time guard preventing dual-backend builds via compile_error!
  * Backend enum with SQLite, NativeV2, Unknown variants
  * Backend::detect() method for file header identification
...
- **Native-v2 schema validation:** Placeholder in status() method - needs implementation via GraphBackend metadata queries.
