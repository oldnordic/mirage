...
fn mirage_status_matches_magellan_cfg_count_on_same_geo() {
    ...
    assert!(stdout.contains("cfg_blocks") || stdout.contains("cfg"),
        "Status should report CFG info. Output: {}", stdout);
}
...
fn mirage_cfg_accepts_unique_simple_name() {
    ...
    assert!(has_output,
        "cfg should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_cfg_accepts_fqn() {
    ...
    let cfg = Command::new(mirage_bin())
        .arg("cfg")
        .arg("--db").arg(&db_path)
        .arg("--function").arg("test_crate::fibonacci")
        .output()
        .expect("Failed to run mirage cfg");
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "cfg with FQN should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_cfg_accepts_numeric_id() {
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "cfg with numeric ID should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_cfg_reports_explicit_ambiguity() {
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "cfg should report result or ambiguity. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_loops_works_on_real_function() {
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "loops should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_dominators_works_on_real_function() {
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "dominators should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_paths_works_on_real_function() {
    ...
    let has_output = !stdout.is_empty() || !stderr.is_empty();
    assert!(has_output,
        "paths should produce output. stdout: {}, stderr: {}", stdout, stderr);
}
...
fn mirage_full_system_smoke() {
    ...
    // Should produce output
    let stdout = String::from_utf8_lossy(&cfg.stdout);
    assert!(!stdout.is_empty() || cfg.status.success(),
